...
// key what, value {t: timeStampMs, d:[{ docId, state: new|dirty | clean, sub, length, data]}
...
io.cache.get = function (key, ageMs) {
    if (ageMs == null) ageMs = io.cacheMsDEFAULT;
...
io.fetch = function (what, action, actionNone, options) {
    var d = io.cache.get(what, options.cacheMs);
    if (d) {
        io.fetchDbLoaded(what, d, action,actionNone, options);
    }
    else {
        io.fetchDb(what, action, actionNone, options);
    };
};

io.fetchDb = function (what, action, actionNone, options) {
    ...
    if (what.orderBy) {
        for (var i = 0; i < what.orderBy.length; i++) {
            query = query.orderBy(what.orderBy[i].field, what.orderBy[i].dir);
        }
    }
    query.get().then((querySnapshot) => {
        ...
        io.cache.set(what, a);
        io.fetchDbLoaded(what, a, action, actionNone, options);
    });
};

io.fetchDbLoaded = function (what, d, action, actionNone, options) {
    var dirtyMet = false;
...
io.signIn = function () {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        if (!user) {
            window.location = "login.htm?url=productManager.htm"; //todo capture this string
        };
    });
};
